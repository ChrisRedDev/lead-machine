      ...
      <main className="p-4 lg:p-6 max-w-lg">
        <div className="border border-border/40 rounded-2xl p-6">
          <div className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-3">
            {credits?.plan || "Basic"} Plan
          </div>
          <div className="text-4xl font-bold font-display tracking-tight mb-1">
            {credits?.balance ?? 0}
          </div>
          <p className="text-sm text-muted-foreground mb-5">credits remaining</p>
          <div className="w-full h-1.5 bg-secondary rounded-full overflow-hidden">
            <div
              className="h-full rounded-full bg-primary transition-all"
              style={{ width: `${Math.min(100, ((credits?.balance ?? 0) / 100) * 100)}%` }}
            />
          </div>
